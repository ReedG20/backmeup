...
export default function RecordScreen() {
  ...
  return (
    <View className="flex-1 bg-white">
      ...
      {/* Content area - flex container for proper button positioning */}
      <View className="flex-1">
        {/* Live transcript area - takes up most but not all space */}
        {isRecording && (
          <ScrollView className="mx-4 my-6 rounded-xl bg-gray-50 p-4" style={{ flex: 0.7 }}>
            {turns.length === 0 ? (
        ...
        {/* Button area - centered with space above when recording */}
        <View className={`px-8 ${isRecording ? 'pb-8 pt-8' : 'flex-1 justify-center'}`}>
          {!isRecording ? (
            <Pressable
